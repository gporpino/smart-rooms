...
it "save reservation limite initial" do
  ...
  it "save reservation higher interval" do
  ...
  it "save reservation interval inside" do

    now = DateTime.now

    reservation = FactoryGirl.build(:reservation, initial_date: now - 1.hour , end_date: now + 1.hour)
    reservation.should be_valid
    reservation.save

    reservation2 = FactoryGirl.build(:reservation, initial_date: now, end_date: now + 10.minutes)
    reservation2.should_not be_valid
  end

   it "save reservation interval outside" do

    now = DateTime.now

    reservation = FactoryGirl.build(:reservation, initial_date: now, end_date: now + 10.minutes)
    reservation.should be_valid
    reservation.save

    reservation2 = FactoryGirl.build(:reservation, initial_date: now - 1.hour , end_date: now + 1.hour)
    reservation2.should_not be_valid
  end
